...
class EvitementObstacle(Node):
    def __init__(self):
        ...
        self.marge = 0.5  # en cm
        self.moveLeftStatus = False
        self.moveRightStatus = False

        self.get_logger().info("Evitement d'obstacle")
    ...
    def odom_callback(self, msg: Odometry):
        ...
        # evitement d'obstacles: on rajoute un point immediat au waypoint
        if self.moveLeftStatus:
            self.waypoints.insert(
                self.currentWaypoints,
                (  # (x, y, theta) en gros le point est a 45° à gaucge en avance de 25cm
                    x + 0.25 * math.cos(theta - math.pi / 4),
                    y + 0.5 * math.sin(theta - math.pi / 4),
                    self.waypoints[self.currentWaypoints][2],
                ),
            )
            self.moveLeftStatus = False
        elif self.moveRightStatus:
            self.waypoints.insert(
                self.currentWaypoints,
                (  # (x, y, theta) en gros le point est a 45° à gaucge en avance de 25cm
                    x + 0.25 * math.cos(theta + math.pi / 4),
    ...
    def scan_callback(self, msg: LaserScan):
        # On regarde devant de 30° à gauche
        for scan in msg.ranges[0:30]:
            if scan > 0.01 and scan <= self.marge:
                # Obstacle detecte du cote gauche -> on va a droite
                self.moveLeftStatus = True
                self.get_logger().info("Obstacle detecte a gauche")

        # On regarde devant de 30° à droite
        for scan in msg.ranges[330:360]:
            if scan > 0.01 and scan <= self.marge:
                # Obstacle detecte du cote droit -> on va a gauche
                self.moveRightStatus = True
                self.get_logger().info("Obstacle detecte a droite")
